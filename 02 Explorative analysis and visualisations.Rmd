...
listing_data = listing_data[1:10,]
listing_data$ID = as.factor(listing_data$ID)

#plot trend of owning airbnb
(top_hosts = ggplot(listing_data, aes(x = reorder(ID, -listing_host), y = listing_host, fill = ID))+
...
#save top 10 IDs for later
top10 = as.numeric(listing_data$ID)
```
...
ggplot(price_data, aes(x = longitude, y = latitude, color = price))+
  geom_point()+
  scale_color_viridis()+
  theme_map()
```
